...
"""
Program: convert_samplesheet.py
Summary:
  This is a program that reads a (front-end) CSV spreadsheet file, performs a variety of checks, 'fixes'
  sample names, and writes a 'back-end' samplesheet file for use with the processing pipeline. There are
  two back-end samplesheet formats: Andrew's sci-ATAC pipeline samplesheet format and a JSON format file
  for the BBI sci-ATAC pipeline.

Input (front-end) samplesheet format:
  o  the samplesheet file is a CSV format spreadsheet file (use a spreadsheet program to create the CSV file)
  o  the first row is a header with the following required columns
       ...
       o  p5 barcode identifier with possible values: 'p5_wells' or 'p5_indexes' or 'p5_columns'
       o  p7 barcode identifier with possible values: 'p7_wells' or 'p7_indexes' or 'p7_rows'
  o  the header values do not depend on case
  o  the column order is arbitrary (but must be consistent within the file)
  o  ranges: index, well, column, and row range values are separated by either '-' or ':', your choice (mixes
     are allowed)
     examples:
       o  3-5
       o  3:5
       o  3-5,7:12
  o  multiple barcode values: multiple indexes, wells, columns, and rows (and ranges) are separated by ','
     examples:
  ...
  o  sample names:
       ...
       o  allowed characters are alphabetic (a-z and A-Z), numeric (0-9), and period '.'
       o  this program silently converts other characters in the sample name to '.', and
          then checks for sample name degeneracy. If there is, the program exits immediately.
  o  genomes:
       o  recognized genome names are listed in the variable 'genome_name_list' in this
          program's code. If a samplesheet genome name is not in the list, convert_samplesheet.py
          gives a warning in case the name is mis-spelled.
       o  genome is the name of the organism that was sequenced. This identifies the files required
          to analyze the reads. The genome string is passed to the processing pipeline, and the
          pipeline uses it to find required files.
  o  wells:
       o  samplesheet wells are converted to indexes where indexes refer to physical wells, which
          are in the order used in Andrew's pipeline; that is, N7 and P7 indexes increase by
          column number along each row and N5 and P5 indexes increase by row letter down each
          column
       o  wells that include a plate identifier can have either '-' or ':' separating the plate
          and well, your choice (mixes within a range are allowed)
          examples:
            o  P1-A10
            o  P1:A10
       ...
       o  when the same set of wells is used from all four plates for a sample, use 'P*' for
          the plate number
          example:
       ...
       o  wells names do not depend on case
          examples:
            ...
            o  p1-a10
  o  indexes:
       o  indexes are given as single or ranges of integer well indexes separated by commas.
          example:
            o  3,5-8,89:96
...
convert_samplesheet.py -h

which gives

usage: convert_samplesheet.py [-h] [-i INPUT] [-o OUTPUT] [-f {json,index}]
                              [-l {2,3}] [-t] [-s {n5,n7}] [-d]
...
optional arguments:
  ...
  -l {2,3}, --level {2,3}
                        Two or three level sci-ATAC-seq experiment (default: '3') (integer).
  -t, --tn5_barcodes    Tn5 has barcodes (flag: no value).
  -s {n5,n7}, --sample_identifier {n5,n7}
                        Ligation barcode that identifies the sample (default: 'n5') used to check for duplicates (string).
  -d, --documentation   Display documentation (flag: no value).
"""
...
def parse_header( row_header ):
  ...
  The dictionary has the elements
    ...
    format  barcode format values: wells, indexes, rows, columns, None (see column_name_values for allowed combinations of type and format)
  """
  column_name_list = []
  error_string = ''
...
def parse_wells( string_in, across_row_first=True, max_index = 96, element_coordinates = [ None, None ] ):
  ...
  for well_range in string_in.split( ',' ):
    ...
    well_range = well_range.upper()
#    mobj = re.match( r'(P([1-4])-)?([A-H])([01]?[0-9])([-:](P([1-4])-)?([A-H])([01]?[0-9]))?$', well_range )
    mobj = re.match( r'([P]([1-4*])[-:])?([A-H])([0]?[1-9][0-2]?)([-:]([P]([1-4*])[-:])?([A-H])([0]?[1-9][0-2]?))?$', well_range )
    if( not mobj ):
...
def read_samplesheet( file ):
  ...
  Notes:
    ...
    o  the column header names must be in the list '`column_name_values'.
...
def get_pcr_wells( column_name_list, samplesheet_row_list ):
  ...
  for samplesheet_row in samplesheet_row_list:
    ...
    for value_range in samplesheet_row[p5_samplesheet_col].split( ',' ):
      value_range = re.sub( r'\s', '', value_range )
      value_range = value_range.upper()
      mobj = re.match( r'([pP]([1-4*])[-:])?([a-hA-H])([0]?[1-9][0-2]?)([-:]([pP]([1-4*])[-:])?([a-hA-H])([0]?[1-9][0-2]?))?$', value_range )
      if( not mobj ):
...
def write_samplesheet_json_format( file, column_name_list, samplesheet_row_list, row_out_list, level = 3, tn5_barcodes = False ):
  """
  Write a samplesheet file in JSON format.
  """
...
if __name__ == '__main__':
  ...
  parser.add_argument('-d', '--documentation', required=False, action='store_true', help='Display documentation (flag: no value).')
  ...
  if( len( error_string ) > 0 ):
    ...
    print( 'For help run \'convert_samplesheet.py -h\' or \'convert_samplesheet.py -d\'' )
    sys.exit( -1 )
  ...
  if( args.format == 'json' ):
    write_samplesheet_json_format( open( filename_out, 'w' ), column_name_list, samplesheet_row_list, row_out_list, level = args.level, tn5_barcodes = args.tn5_barcodes )
  else:
